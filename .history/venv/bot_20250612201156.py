...
async def handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not mode:
        await update.message.reply_text("❗ Пожалуйста, введите /start для начала опроса или /admin для входа в админ-панель.")
        return

    if text.lower() in ["отмена", "🔙 отмена", "🔙", "/cancel"]:
        context.user_data.clear()
        reply_markup = ReplyKeyboardMarkup(admin_keyboard, resize_keyboard=True) if user_id in ADMIN_ID else None
