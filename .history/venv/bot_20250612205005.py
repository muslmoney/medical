...
async def cancel_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply_markup = ReplyKeyboardMarkup(admin_keyboard, resize_keyboard=True) if user_id in ADMIN_ID else ReplyKeyboardRemove()
    await update.message.reply_text("❌ Опрос отменён.", reply_markup=reply_markup)
...
async def handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "⚠️ Непонятная команда или сообщение.\nНажмите /start, чтобы начать заново."
    )
