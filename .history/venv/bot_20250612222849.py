...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🌐 Пожалуйста, выберите язык / Iltimos, tilni tanlang / Please choose a language:",
...
async def handle_language_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if "lang" in context.user_data:
        return  # язык уже выбран, пропускаем
    ...
    if text in LANGUAGES:
        ...
        await update.message.reply_text("⚠️ Пожалуйста, выберите язык из списка.", reply_markup=language_keyboard)
...
async def handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in ADMIN_ID:
        ...
        if action == "add_question_text":
            ...
            await update.message.reply_text(
                "📌 Укажите тип вопроса: choice (варианты) или text (свободный ввод):",
            ...
            if text == "choice":
                ...
                new_q = {
                    ...
                    "text": {context.user_data["lang"]: context.user_data["new_q_text"]},
                    "type": "text",
                    "options": {}
                }
            ...
            new_q = {
                ...
                "text": {context.user_data["lang"]: context.user_data["new_q_text"]},
                "type": "choice",
                "options": {context.user_data["lang"]: options}
            }
            data["questions"].append(new_q)
            save_data(data)
            context.user_data.clear()
    ...
    if "lang" not in context.user_data:
        await update.message.reply_text("⚠️ Пожалуйста, выберите язык: /start")
        return
...
if __name__ == "__main__":
    ...
    # Обработка языка – сначала
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_language_selection), group=0)

    # Остальные сообщения – потом
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_answer), group=1)
