...
async def send_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step >= len(data["questions"]):
        ...
        save_data(data)
        await update.message.reply_text(LANG_TEXTS["thanks"][lang], reply_markup=ReplyKeyboardRemove())
        return
    ...
    if q["type"] == "choice":
        opts = q["options"].get(lang, q["options"]["ru"])
        keyboard = [[opt] for opt in opts] + [[LANG_TEXTS["other"][lang]], [LANG_TEXTS["cancel"][lang]]]
        markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=True)
    else:
        markup = ReplyKeyboardMarkup([[LANG_TEXTS["cancel"][lang]]], resize_keyboard=True)
